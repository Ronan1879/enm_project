...
Ca_val = []
Cb_val = []
Ca_val.append((1 - (R*sigma[0]/ep_r[0])))
Ca_val.append((1 - (R*sigma[1]/ep_r[1])))
Cb_val.append(Ra/(ep_r[0]))
Cb_val.append(Ra/(ep_r[1]))
...
for n in range(Nmax):
    ...
    # 3.93 d)
    # bad values are [:,0,0]
    #E_temp[:,:,:,0] = Ca_arr*E[:,:,:,0] + Cb_arr/Rb*(H[:,:,:,2] - np.roll(H[:,:,:,2],1,axis=1) - H[:,:,:,1] + np.roll(H[:,:,:,1],1,axis=2))
    # 3.93 e)
    # bad values are [0,:,0]
    #E_temp[:,:,:,1] = Ca_arr*E[:,:,:,1] + Cb_arr/Rb*(H[:,:,:,0] - np.roll(H[:,:,:,0],1,axis=2) - H[:,:,:,2] + np.roll(H[:,:,:,2],1,axis=0))

    # 3.93 f)
    # bad values are [0,0,:]
    E_temp[:,:,:,2] = Ca_arr*E[:,:,:,2] + Cb_arr/Rb*(H[:,:,:,1] - np.roll(H[:,:,:,1],1,axis=0) - H[:,:,:,0] + np.roll(H[:,:,:,0],1,axis=1))
...
ani = animation.FuncAnimation(
    fig, animate, interval=30, blit=False, save_count=1000)
